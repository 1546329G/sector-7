...
const AdministrarAsistencia: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchPeriodos() {
      try {
        const res = await fetch(`${API_URL}/reporte-asistencia/periodos`);
        const data = await res.json();
  ...
  useEffect(() => {
    async function fetchSemanas(periodoStr: string) {
      ...
      try {
        const res = await fetch(`${API_URL}/reporte-asistencia/semanas?periodo=${periodoStr}`);
        const data = await res.json();
...
// Cargar datos de un solo profesor
  const loadSingleProfessorData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/reporte-asistencia?inicio=${inicio}&fin=${fin}&modo=periodo&profesor_id=${selectedProfessorId}`);
...
const loadAllProfessorsForWeek = useCallback(async () => {
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/reporte-asistencia?inicio=${inicio}&fin=${fin}&modo=semana`);
    const json = await res.json();
