...
// Interfaz para un objeto Horario que coincide con la API de Horarios y la tabla 'horario'
interface Horario {
    id: string; // varchar(50) en DB
    id_profesor: number; // int en DB
    profesor_nombre?: string; // Propiedad adicional para mostrar el nombre en la tabla
    dia_semana: 'Lunes' | 'Martes' | 'Miércoles' | 'Jueves' | 'Viernes' | 'Sábado'; // varchar(50) en DB
    hora_entrada: string; // time en DB
    hora_salida: string;  // time en DB
    aula: string; // La API de horarios GET devuelve 'aula', aunque no está en describe horario, asumimos que se usa o se generará
    estado?: string; // varchar(50) en DB, puede ser NULL
    fecha_registro?: string; // datetime en DB, puede ser NULL
    fecha_modificacion?: string; // datetime en DB, puede ser NULL
}

// Interfaz simplificada para el profesor, basada en la API de Profesores y la tabla 'profesor'
interface ProfesorSimple {
    id: number; // int en DB, auto_increment
    nombre: string; // varchar(255) en DB
...
const GestionHorarios: React.FC = () => {
    ...
    const [newProfesorId, setNewProfesorId] = useState(''); // Se mantiene como string para el select HTML
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const fetchedHorarios = horariosResponse.data; // Datos de la API de horarios
            const fetchedProfesores = profesoresResponse.data; // Datos de la API de profesores

            setProfesores(fetchedProfesores); // Actualiza el estado de profesores para el <select> del formulario
    ...
    }, []); // fetchData es una dependencia de useEffect, pero useCallback sin dependencias asegura que no se recree innecesariamente.
    ...
    const handleAddHorario = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Datos a enviar al backend para el nuevo horario
            const newHorarioData = {
                id_profesor: profesorIdAsNumber, // De acuerdo a la tabla 'horario'
                dia_semana: newDiaSemana, // De acuerdo a la tabla 'horario'
                hora_entrada: newHoraEntrada + ':00', // Agregamos ':00' para asegurar formato HH:MM:SS si la DB lo espera
                hora_salida: newHoraSalida + ':00',   // Agregamos ':00' para asegurar formato HH:MM:SS si la DB lo espera
                aula: newAula, // La API de horarios GET devuelve 'aula'
                estado: "Activo" // Añadir un estado por defecto si el backend lo espera y no lo genera automáticamente
            };
            ...
            // Después de añadir, vuelve a obtener todos los datos para tener la lista actualizada
            fetchData();
            ...
            if (axios.isAxiosError(error) && error.response) {
                // Errores de respuesta del servidor (HTTP 4xx, 5xx)
                ...
                // Errores de red o de cliente antes de que la solicitud llegue al servidor
    ...
    const handleDeleteHorario = async (horarioId: string) => {
        ...
        try {
            ...
            // Después de eliminar, vuelve a obtener todos los datos para asegurar la consistencia
            fetchData();
