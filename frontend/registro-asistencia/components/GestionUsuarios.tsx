...
const GestionUsuarios: React.FC = () => {
    ...
    const [newRole, setNewRole] = useState<User['rol']>('usuario');
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            setMessage({ type: 'error', text: error.message || 'Error al cargar usuarios. Asegúrate de que tu API tiene el endpoint /api/users y estás autenticado.' });
            setUsers([]);
        } finally {
            setIsLoading(false);
        }
    }, [getToken, API_USERS_URL]); // Añadir API_USERS_URL a las dependencias

    // Cargar usuarios al montar el componente
    // Se ejecuta una vez al inicio y cada vez que fetchUsers cambia (lo cual solo será si getToken cambia,
    // es decir, rara vez después del mount inicial).
    useEffect(() => {
        fetchUsers();
    }, [fetchUsers]);
